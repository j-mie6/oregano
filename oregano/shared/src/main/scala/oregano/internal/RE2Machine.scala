...
class RE2Machine(val prog: Prog) extends Machine {
    ...
    def submatches: Array[Int] = if ncap == 0 then Array.empty[Int] else Array.copyOf(matchcap, ncap)
    ...
    def free(t: RE2Thread): Unit = {
        if (pool.length <= poolSize) pool = Array.copyOf(pool, pool.length * 2)
        pool(poolSize) = t
        poolSize += 1
    }
    ...
    def add(pc: Int, pos: Int, cap: Array[Int], q: RE2Queue, t: RE2Thread | Null): RE2Thread | Null = {
        if (pc == 0) t
            ...
            inst.op match {
                case InstOp.FAIL => t
                case InstOp.NOP => add(inst.out, pos, cap, q, t)
                ...
                case InstOp.MATCH | InstOp.RUNE | InstOp.RUNE1 | InstOp.RUNE_ANY | InstOp.RUNE_ANY_NOT_NL =>
                    ...
                    if (ncap > 0 && (thread.cap ne cap)) {
                        Array.copy(cap, 0, thread.cap, 0, ncap)
                    }
                    q.denseThreads(d) = thread
                    null

                case _ => throw new IllegalStateException(s"bad inst: ${inst.op}")
            }
        }
    }

    def step(runq: RE2Queue, nextq: RE2Queue, pos: Int, rune: Int): Boolean = {
        ...
        while (j < runq.size) {
            ...
            if (t != null) {
                ...
                inst.op match {
                    ...
                    case InstOp.RUNE => addNext = inst.matchRune(rune)
                    case InstOp.RUNE1 => addNext = rune == inst.runes(0)
                    case InstOp.RUNE_ANY => addNext = true
                    case InstOp.RUNE_ANY_NOT_NL => addNext = rune != '\n'
                    case _ => throw new IllegalStateException(s"bad inst: ${inst.op}")
                }
                if (addNext) t = add(inst.out, pos + 1, t.cap, nextq, t)
                if (t != null) {
                    free(t)
    ...
    def matches(input: CharSequence): Boolean = {
        ...
        while (!runq.isEmpty) {
            val rune = if pos < input.length then input.charAt(pos).toInt else -1 // EOF
        ...
        if (anchorEnd) matchcap(1) == input.length else matchcap(1) != -1
    }
}

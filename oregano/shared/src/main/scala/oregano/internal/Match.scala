...
trait MatchData {
    ...
    def matched: String = if start >= 0 then source.subSequence(start, end).toString else null
    def group(i: Int): String = if start(i) >= 0 then source.subSequence(start(i), end(i)).toString else null
    def subgroups: List[String] = (1 to groupCount).toList.map(group(_))
    def before: CharSequence = if start >= 0 then source.subSequence(0, start) else null
    def before(i: Int): CharSequence = if start(i) >= 0 then source.subSequence(0, start(i)) else null
    def after: CharSequence = if end >= 0 then source.subSequence(end, source.length) else null
    def after(i: Int): CharSequence = if end(i) >= 0 then source.subSequence(end(i), source.length) else null

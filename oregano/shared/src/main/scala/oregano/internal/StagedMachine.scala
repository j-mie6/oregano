...
object StagedMachine {
    ...
    def generateStepLoop(prog: Prog)(using Quotes): Expr[(RE2Machine, CharSequence) => Boolean] = {
        ...
        def caseDefs(m: Expr[RE2Machine], runq: Expr[RE2Queue], nextq: Expr[RE2Queue], pos: Expr[Int], rune: Expr[Int]/*, addDispatcher: Expr[(Int, Array[Int], Int, RE2Queue, RE2Thread) => RE2Thread]*/)(using Quotes): List[CaseDef] = {
            ...
            (0 until prog.numInst).toList.map { pc =>
                ...
                val body = generateStagedStepForPc(pc, inst, prog, m, runq, nextq, pos, rune)
        ...
        '{ (m: RE2Machine, input: CharSequence) =>
            ...
            val _ = ${generateInlineAdd(prog.start, prog.getInst(prog.start), prog, 'm, 'clone, '{0}, 'runq, '{null})}

            // val _ = m.add($progStartExpr, 0, m.matchcap.clone(), runq, null)
            ...
            while (!runq.isEmpty) {
                ...
                while (i < runq.size) {
                    ...
                    ${Match('pc.asTerm, caseDefs('m, 'runq, 'nextq, 'pos, 'rune)).asExprOf[Unit]}
    ...
    def generateStepHandlers(prog: Prog)(using Quotes): Expr[Array[(Int, Int, RE2Queue, RE2Queue, Int, RE2Machine) => Unit]] = {
        ...
        (0 until prog.numInst).toList.map { pc =>
            val inst = prog.getInst(pc)
            '{ (rune: Int, pos: Int, runq: RE2Queue, nextq: RE2Queue, _: Int, m: RE2Machine) => ${generateStagedStepForPc(pc, inst, prog, 'm, 'runq, 'nextq, 'pos, 'rune)} }
        }

        '{ Array(${ Varargs(handlerExprs) }*) }
    }

    def generateStagedStepForPc(pc: Int, inst: Inst, prog: Prog, m: Expr[RE2Machine], runq: Expr[RE2Queue], nextq: Expr[RE2Queue], pos: Expr[Int], rune: Expr[Int]/*, addDispatcher: Expr[(Int, Array[Int], Int, RE2Queue, RE2Thread) => RE2Thread]*/)(using Quotes): Expr[Unit] = {
        val pcExpr = Expr(pc)

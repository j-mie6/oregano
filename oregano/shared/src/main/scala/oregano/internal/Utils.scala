...
private object Utils {
    def countNodes(expr: Expr[Any])(using Quotes): Int = {
        ...
        tree match
            ...
            case Match(selector, cases) =>
                1 + loop(selector) + cases.map { case CaseDef(_, guard, rhs) =>
    ...
    inline def dietToRanges(diet: Diet[Int]): List[Int] = diet.toIterator.foldRight(List.empty[Int])((range, acc) => range.start :: range.end :: acc).toList
